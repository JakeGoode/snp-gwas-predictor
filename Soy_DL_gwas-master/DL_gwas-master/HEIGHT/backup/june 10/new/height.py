...
def sync_folds_column(source_file, target_file, output_file=None, id_col=1):
	"""
	Copy the first column (fold assignment) from source_file to target_file by matching on sample ID.
	"""
	...
	# Match by ID column
	source_ids = source_df.iloc[:, id_col]
	target_ids = target_df.iloc[:, id_col]

	# Create mapping from ID to fold
	id_to_fold = dict(zip(source_ids, source_df.iloc[:, 0]))

	# Apply mapping to target
	new_folds = target_ids.map(id_to_fold)

	if new_folds.isnull().any():
		missing = target_ids[new_folds.isnull()]
		raise ValueError(f"Some sample IDs in {target_file} were not found in {source_file}:\n{missing}")

	target_df.iloc[:, 0] = new_folds
...
def readData(input):
	...
	SNP_onehot = np.array([indices_to_one_hot(row, nb_classes) for row in SNP], dtype=np.float32)

	return SNP, SNP_onehot, pheno, folds, snp_names
...
def resnet(input):
	...
	x = Conv1D(64,4,padding='same',activation = 'linear',kernel_initializer = 'TruncatedNormal', kernel_regularizer=regularizers.l2(1e-3),bias_regularizer = regularizers.l2(1e-4))(inputs)
	
	x = Conv1D(64,20,padding='same',activation = 'linear', kernel_initializer = 'TruncatedNormal',kernel_regularizer=regularizers.l2(1e-3),bias_regularizer = regularizers.l2(1e-4))(x)
		
	x = Dropout(0.5)(x)
	
	shortcut = Conv1D(64,4,padding='same',activation = 'linear',kernel_initializer = 'TruncatedNormal', kernel_regularizer=regularizers.l2(1e-3),bias_regularizer = regularizers.l2(1e-4))(inputs)
	x = layers.add([shortcut,x])
	
	x = Conv1D(64,4,padding='same',activation = 'linear',kernel_initializer = 'TruncatedNormal', kernel_regularizer=regularizers.l2(1e-3),bias_regularizer = regularizers.l2(1e-4))(x)
	...
	outputs = Dense(1,activation = isru,bias_regularizer = regularizers.l2(1e-4),kernel_initializer = 'TruncatedNormal',name = 'out')(x)
...
class SNPGenerator(tf.keras.utils.Sequence):
	def __init__(self, onehot_data, labels, batch_size, **kwargs):
		super().__init__(**kwargs)
		self.SNP = onehot_data
		self.labels = labels
		self.batch_size = batch_size
...
def model_train(testX, valX, trainX, testY, valY, trainY, model_save, weights_save):
	batch_size = 256
	early_stopping = keras.callbacks.EarlyStopping(monitor='val_mae', patience=20, mode='min')
	...
	train_gen = SNPGenerator(trainX, trainY, batch_size)
	val_gen = SNPGenerator(valX, valY, batch_size)
	...
	# Test set: already one-hot encoded
	pred = model.predict(testX)
	pred = pred.flatten()

	corr = pearsonr(pred, testY)[0]
...
def run_saliency_summary(IMP_input, QA_input):
	...
	imp_SNP, imp_onehot, imp_pheno, folds, snp_names = readData(IMP_input)
...
def main(IMP_input, QA_input, run_fold=None):
	...
	imp_SNP, imp_onehot, imp_pheno, folds, snp_names = readData(IMP_input)
	QA_SNP, QA_onehot, QA_pheno, folds, _ = readData(QA_input)
	PHENOTYPE = imp_pheno
	...
	for i in fold_range:
		...
		# trainSNP, trainSNP_QA, trainPheno = imp_onehot[trainIdx], QA_onehot[trainIdx], PHENOTYPE[trainIdx]
		...
		trainX, trainX_QA, trainY = imp_onehot[trainIdx], QA_onehot[trainIdx], PHENOTYPE[trainIdx]
		valX, valX_QA, valY = imp_onehot[valIdx], QA_onehot[valIdx], PHENOTYPE[valIdx]
		testX, testX_QA, testY = imp_onehot[testIdx], QA_onehot[testIdx], PHENOTYPE[testIdx]
		...
		# )
		history, corr = model_train(
    		testX, valX, trainX, testY, valY, trainY,
    		f'model_IMP/model_{i}.h5',
    		f'model_IMP/model_weights{i}.weights.h5'
		)
		...
		# )
		history, corr = model_train(
    		testX_QA, valX_QA, trainX_QA, testY, valY, trainY,
    		f'model_IMP/model_{i}.h5',
    		f'model_IMP/model_weights{i}.weights.h5'
		)
